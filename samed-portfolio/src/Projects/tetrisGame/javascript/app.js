...
let colorList = ['#CC99C9', '#9EC1CF', '#9EE09E', '#FDFD97', '#FEB144', '#FF6663'],
    currentColor;
...
window.addEventListener('keydown', () => {
    ...
    currentShapePos.forEach(value => {
        if (grid[value.y][value.x - 1] == 'o')
            tempAllowLeft = false;
        if (grid[value.y][value.x + 1] == 'o')
            tempAllowRight = false;
    ...
    if (allowReposition && !pauseGame) {
        if (event.key == 'a' || event.key == 'd' || event.key == 'ArrowRight' || event.key == 'ArrowLeft') {
            removeShape();
        }
...
setInterval(() => {
    if (!pauseGame) {
        ...
        if (maxPosition.bottom < 23)
            for (let index = maxPosition.left; index < maxPosition.right + 1; index++)
        ...
        currentShapeFirstBlockPos.forEach((value) => {
            if (maxPosition.bottom < 23 && !bottomBump) {
                ...
                for (let maxHeight = 0; maxHeight < (maxPosition.bottom - maxPosition.top) + 1; maxHeight++) {
                    blowLine();
                }
...
function getMaxPos(positionsX, positionsY) {
    ...
    for (let x = 0; x < positionsY.length; x++) {
        for (let y = 0; y < positionsY.length; y++) {
            if (positionsY[x] > positionsY[y])
                maxPosition.bottom = positionsY[x];
        }
    }
}
    ...
    for (let y = 4; y < 24; y++) {
        for (let x = 0; x < 10; x++) {
            if (grid[y][x] == '') {
                ...
                gameContainer.innerHTML +=
                    `<div class="game-block" style="background-color: ${currentColor};"></div>`;
            } else if (grid[y][x] != 'x' && grid[y][x] != '') {
                gameContainer.innerHTML +=
                    `<div class="game-block" style="background-color: ${colorGrid[y][x]}"></div>`;
            }
        }
    }
}
...
function drawShapeGrid(shape, defaultPos, x, y) {
    ...
    switch (shape) {
        ...
        case 'O':
            currentShapeFirstBlockPos.splice(0, 4);
            for (let yPos = y; yPos < y + 1; yPos++) {
                for (let blockQuantityY = y; blockQuantityY < y + 2; blockQuantityY++) {
                    for (let blockQuantityX = x; blockQuantityX < x + 2; blockQuantityX++) {
        ...
        case 'L':
            currentShapeFirstBlockPos.splice(0, 4);
            for (let yPos = y; yPos < y + 1; yPos++) {
                for (let blockQuantityY = y; blockQuantityY < y + 3; blockQuantityY++) {
